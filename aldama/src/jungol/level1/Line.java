 ...
 * 
 * 예를 들어 5명의 학생이 줄을 서고 첫 번째로 줄을 선 학생부터 다섯 번째로 줄을 선 학생까지 차례로 0 1 1 3 2번의 번호를 뽑았다고
 * 하자 첫 번째 학생부터 다섯 번째 학생까지 1부터 5로 표시하면 학생들이 줄을 선 순서는 다음과 같이 된다. 
 * 첫 번째 학생이 번호를 뽑은 후 : 1 
 ...
 * [출력]
 * 학생들이 처음에 줄을 선 순서대로 1번부터 번호를 매길 때 첫째 줄에 학생들이 최종적으로 줄을 선 순서를 그 번호로 출력한다. 학생 번호
 * 사이에는 한 칸의 공백을 출력한다.
 * 
 * @author KooKoo
 *
...
public class Line {
	public static void main(String[] args) throws Exception {
		System.setIn(new FileInputStream("resources/jungol/level1_line_input.txt"));
		...
		for(int n=0;n<N;n++){
			pos = sc.nextInt();
			while(pos-- > 0){
				buffer.push(line.pop());
			}
			line.push(n+1);
			while(buffer.size() > 0){
				line.push(buffer.pop());
			}
		}
		
		while(line.size() > 0) {
			System.out.printf("%d ",line.pollLast());
		}
		System.out.println();
	}
}
